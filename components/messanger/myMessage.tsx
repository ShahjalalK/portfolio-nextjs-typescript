...
const MmyMessage = ({item}: Props) => {
  ...
  return (
    ...
    {basicInfoValue.slice(0, 1).map((me) => (
         ...
         <div className="whitespace-normal text-sm bg-[#D9D9D7]/10 p-2 rounded-r-md rounded-tl-md ml-5 font-normal text-white border border-white/30 flex flex-col space-y-1 max-w-[16rem] text-white/80 hover:text-white/90">
        <p> {item.message}</p>
          {item.media && <div className="cursor-pointer">
          <Link href={item.media} download={true} target="_blank"><Image src={item.media} alt={item.name} width={350} height={350} className="w-full h-auto"/></Link>
          </div>}
           </div>  
