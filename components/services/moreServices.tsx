...
const MoreServices = ({slugId}: Props) => {
  ...
  return (
    <div className="my-16 container">
        ...
        <Swiper        
        className=" max-h-[20.8rem] md:max-h-[16rem]"
        breakpoints={{          
          640: {
            slidesPerView: 1,
            spaceBetween: 0,
            ...
            allService.filter(item => item.ServicePath !== slugId).map((service, index) => (
              <SwiperSlide key={index}>
                {basicInfo.slice(0, 1).map((me) => (
                   <motion.div key={me._id} initial={{y : 100}} whileInView={{y : 0}} transition={{duration : 0.5}} viewport={{once : true}}>
                   <Link href={`/service/${service.ServicePath}`} className="group " >
                    <Image src={service.serviceImage}  alt="signature" width={750} height={750} className=" z-10 rounded-xl border border-primary/25"/>
                     ...
                     <div className="flex items-center space-x-1">
                       <Image src={me.myImage} width={50} height={50} className="w-7 h-7 rounded-full object-cover" alt="me" />
        ...
        <div className=" absolute top-[40%] md:top-[50%] -translate-y-[50%] z-20 flex items-center justify-between w-full">
