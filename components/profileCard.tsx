...
const ProfileCard = ({}: Props) => {
  ...
  return (
    ...
    {basicInfo.slice(0, 1).map((item) => (
      <div key={item._id} className="border border-white/75 rounded-2xl p-5 ">
      ...
      <Link href={item.hireLink} target="_blank" className=" button w-full flex justify-center text-white py-3 mt-5"><span className="flex items-center space-x-2"><HiOutlineMail className="text-white text-2xl" /> <span> Hire Me</span></span><i></i></Link>
  </div>
    ))}
